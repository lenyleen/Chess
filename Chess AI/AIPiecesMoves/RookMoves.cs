...
{
    public class RookMoves : IAIMove
    {
        private readonly List<(int, int)> _moves = new List<(int, int)>() {(1, 0), (-1, 0), (0, 1), (0, -1)};
        public List<AITurn> GetPossibleMoves(AIPiece[][] board, PieceColor color, (int, int) position, AITurn.Pool turnsPool)
        {
            ...
            foreach (var move in _moves)
            {
                ...
                while (t < maxLength)
                {
                    ...
                    if(board[toPlacePosition.Item1][toPlacePosition.Item2] is not null && 
                       board[toPlacePosition.Item1][toPlacePosition.Item2].Color == color) break;
                    possibleMove.Add(turnsPool.Spawn(position,toPlacePosition,PieceType.Rook,board[toPlacePosition.Item1][toPlacePosition.Item2]));
